...
namespace IEDExplorer
{
    public class MyListView : ListView
    {
        ...
        private class CustomCellAddInfo
        {
            public CustomCellType Type { get; set; }
            public int MaxLength { get; set; }
            public int MinVal { get; set; }
            public int MaxVal { get; set; }
            public StringCollection ItemList { get; set; }
        ...
        /// <summary>
        /// NumericUpDown to display in the associated cells
        /// </summary>
        private NumericUpDown numUpDown = new NumericUpDown();
		...
		private void InitializeComponent()
		{
            ...
            this.numUpDown.KeyPress += new KeyPressEventHandler(numUpDown_KeyPress);
            this.numUpDown.ValueChanged += new EventHandler(numUpDown_ValueChanged);            
		}
		...
		///					  given "row" variable.
		///	</param>
		public void AddEditableCell( int row, int col, int len, char[] allowedchars)
		{
			// Add the cell into the hashtable
			// Value is setting as null because it is an editable cell
            this.customCells[new SubItem(row, col)] = new CustomCellAddInfo(CustomCellType.TextBox, len, allowedchars);
		}
		...
		public void AddComboBoxCell( int row, int col, StringCollection data )
		{
            ...
            this.customCells[new SubItem(row, col)] = new CustomCellAddInfo(CustomCellType.ComboBox, data);
		}

        public void AddNumericUpDownCell(int row, int col, int min, int max)
        {
            // Add the cell into the hashtable
            // Value for the hashtable is the combobox items
            this.customCells[new SubItem(row, col)] = new CustomCellAddInfo(CustomCellType.NumericUpDown, min, max);
        }
        ...
        /// <param name="length"> MaxLegth</param>
        ...
        private void ShowNumericUpDown(Point location, Size sz, int min, int max)
        {
            try
            {
                ...
                this.numUpDown.ValueChanged -= new EventHandler(numUpDown_ValueChanged);     
                numUpDown.Size = sz;
                numUpDown.Location = location;
                numUpDown.Minimum = min;
                numUpDown.Maximum = max;
                // Set value, take it from the current listview cell       
                if(Decimal.TryParse(this.Items[row].SubItems[col].Text, out decOut))
                    numUpDown.Value = decOut;
                this.numUpDown.ValueChanged += new EventHandler(numUpDown_ValueChanged);     
                // Show the numericUpDown
                numUpDown.Show();
                numUpDown.Focus();
            }
            catch (ArgumentOutOfRangeException)
			...
			try
			{
				...
				if( this.row != -1 && this.col != -1 )
				{
					...
					if( cell != null)
					{
						...
						if((this.customCells[cell] != null) && (this.customCells[cell] is CustomCellAddInfo))
						{
                            switch ((this.customCells[cell] as CustomCellAddInfo).Type)
                            {
                                case CustomCellType.TextBox:
                                    this.ShowTextBox(location, sz);
                                    break;
                                case CustomCellType.ComboBox:
                                    this.ShowComboBox(location, sz, (StringCollection)(this.customCells[cell] as CustomCellAddInfo).ItemList);
                                    break;
                                case CustomCellType.NumericUpDown:
                                    this.ShowNumericUpDown(location, sz, (this.customCells[cell] as CustomCellAddInfo).MinVal, (this.customCells[cell] as CustomCellAddInfo).MaxVal);
                                    break;
                            }
		...
		private void textBox_Leave(object sender, EventArgs e)
		{
			...
			try
			{
				if( this.row != -1 && this.col != -1 )
				{
                    ...
                    if(launchHandler)
                        this.OnAfterLabelEdit(new LabelEditEventArgs(row));
				}
			}
			catch( Exception ex )
			{
        ...
        private void textBox_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
            ...
            {                
                ...
                {
                    SubItem cell = GetKey(new SubItem(this.row, this.col ));
                    if ((this.customCells[cell] != null) && (this.customCells[cell] is CustomCellAddInfo) && (this.customCells[cell] as CustomCellAddInfo).allowedChars != null)
                    {
                        bool charAllowed = false;
                        foreach (char ch in (this.customCells[cell] as CustomCellAddInfo).allowedChars)
                        {
                            if (ch == e.KeyChar)
                                charAllowed = true;
                        }

                        if(!charAllowed && e.KeyChar != 0x08)
                            e.Handled = true;
                    }
                }
            }
        ...
        private void numUpDown_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)27)
            {
                try
                {
                    ...
                    {
                        this.numUpDown.ValueChanged -= new EventHandler(numUpDown_ValueChanged);
                        this.numUpDown.Value = Convert.ToDecimal(this.Items[row].SubItems[col].Text);
                        this.numUpDown.Hide();
                        this.numUpDown.ValueChanged += new EventHandler(numUpDown_ValueChanged);
                    }
                }
        ...
        private void numUpDown_ValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                if (this.row != -1 && this.col != -1)
                {
                    if (this.Items[row].SubItems[col].Text != this.numUpDown.Value.ToString())
                        launchHandler = true;

                    this.Items[row].SubItems[col].Text = this.numUpDown.Value.ToString();
                    //this.combo.Visible = !this.hideComboAfterSelChange;
